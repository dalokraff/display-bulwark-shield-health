...
local function get_shield_health(unit, difficulty_tweaks)
    local shield_health
        ...
        for unit, data in pairs(mod.shields) do
            ...
            if Unit.alive(unit) then
                if BLACKBOARDS[unit] then
                    ...
                    local shield_health = get_shield_health(unit, difficulty_tweaks)
                    ...
                    if (shield_health or 0) > 0 then
                        if mod.gui then
                            ...
                            data.gui_id = Gui.text_3d(mod.gui, stagger_text, font_material, font_size, font, m, Vector3(0,0,0), 3, color)
                        end
                    end
                else
